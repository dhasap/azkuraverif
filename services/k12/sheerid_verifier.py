...
class SheerIDVerifier:
    """Verifikator Identitas Guru SheerID (Async)"""
    ...
    def __init__(self, verification_id: str):
        ...
        # Client dibuat saat needed atau pakai context manager di verify
    ...
    async def _sheerid_request(self, client: httpx.AsyncClient, method: str, url: str,
    ...
    async def _upload_to_s3(self, client: httpx.AsyncClient, upload_url: str, content: bytes, mime_type: str) -> bool:
        """
        Upload file ke S3 (Async)
        """
        try:
            headers = {
    ...
    async def verify(self, first_name: str = None, last_name: str = None,
               email: str = None, birth_date: str = None,
               school_id: str = None,
               hcaptcha_token: str = None, turnstile_token: str = None) -> Dict:
        """
        Menjalankan proses verifikasi lengkap (Async)
        """
        import asyncio
        try:
            ...
            # PDF is synchronous CPU bound, run in thread
            pdf_data = await asyncio.to_thread(generate_teacher_pdf, first_name, last_name)
            # PNG is async
            png_data = await generate_teacher_png(first_name, last_name)
            ...
            async with httpx.AsyncClient(timeout=30.0) as client:
                ...
                if not await self._upload_to_s3(client, pdf_upload_url, pdf_data, 'application/pdf'):
                    raise Exception("Upload PDF gagal")
                if not await self._upload_to_s3(client, png_upload_url, png_data, 'image/png'):

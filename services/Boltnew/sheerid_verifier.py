...
class SheerIDVerifier:
    """Verifikator Identitas Guru SheerID (Async - Bolt)"""
    ...
    async def create_verification(self, client: httpx.AsyncClient) -> str:
        """Mengajukan verificationId baru melalui installPageUrl (Async)"""
        body = {
            "programId": config.PROGRAM_ID,
            "installPageUrl": self.install_page_url,
        }
        data, status = await self._sheerid_request(
            client, "POST", f"{config.MY_SHEERID_URL}/rest/v2/verification/", body
        )
    ...
    async def verify(
        ...
        """Menjalankan proses verifikasi guru (Async)"""
        try:
            ...
            # Membuat PNG guru (Async)
            logger.info("Langkah 1/5: Membuat dokumen PNG guru...")
            assets = await generate_images(first_name, last_name, school_id)
            for asset in assets:
                logger.info(
            ...
            async with httpx.AsyncClient(timeout=30.0) as client:
                if not self.verification_id:
                    ...
                    await self.create_verification(client)
                ...
                # Mengirim informasi guru
                logger.info("Langkah 2/5: Mengirim informasi guru...")
                step2_body = {
                ...
                if step4_status != 200 or not isinstance(step4_data, dict) or not step4_data.get("documents"):
                    raise Exception(f"Tidak bisa mendapatkan URL upload: {step4_data}")

                documents = step4_data["documents"]
                if len(documents) != len(assets):
                    raise Exception("Jumlah tugas upload yang dikembalikan tidak cocok dengan jumlah file")

                for doc, asset in zip(documents, assets):
                    upload_url = doc.get("uploadUrl")
                    if not upload_url:
                        raise Exception("URL upload tidak ada")
                    if not await self._upload_to_s3(client, upload_url, asset["data"]):
                        raise Exception(f"Gagal upload S3: {asset['file_name']}")
                    logger.info(f"✅ Sudah diupload {asset['file_name']}")
                ...
                logger.info(f"✅ Dokumen selesai dikirim: {getattr(step6_data, 'get', lambda k, d=None: d)('currentStep')}")

                # Mendapatkan status akhir (termasuk rewardCode)
                final_status, _ = await self._sheerid_request(
                    client,
                    "GET",
...
def main():
    ...
    print("=" * 60)
    print("Tool Verifikasi Identitas Guru SheerID (Versi Python)")
    print("=" * 60)
    print()
    ...
    if len(sys.argv) > 1:
        ...
        url = input("Masukkan link SheerID verifikasi (dengan externalUserId): ").strip()
    ...
    print(f"👉 Menggunakan link: {verifier.install_page_url}")
